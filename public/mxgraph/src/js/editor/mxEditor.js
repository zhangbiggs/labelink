...
mxEditor.prototype.addActions = function () {
  ...
  this.addAction('zoomIn', function (editor, factor) {
    console.log(factor)
    editor.graph.zoomIn(factor)
  })

  this.addAction('zoomOut', function (editor, factor) {
    console.log(factor)
    editor.graph.zoomOut(factor)
  })

  this.addAction('center', function (editor) {
    editor.graph.center()
  })
  ...
  this.addAction('FontSizeAdd', function (editor) {
    if (editor.graph.isEnabled()) {
      var fontSize
      var value = editor.graph.getSelectionModel().cells[0].style.split(';')
      value.forEach(item => {
        let arr = item.split('=')
        if (arr.length === 2 && arr[0] === 'fontSize') {
          fontSize = parseInt(arr[1])
        }
      })
      if (!fontSize) {
        fontSize = editor.graph.getStylesheet().styles.text.fontSize
      }
      fontSize += 35.8
      editor.graph.setCellStyles(mxConstants.STYLE_FONTSIZE, fontSize)
    }
  })

  this.addAction('FontSizeCut', function (editor) {
    if (editor.graph.isEnabled()) {
      var fontSize
      var value = editor.graph.getSelectionModel().cells[0].style.split(';')
      value.forEach(item => {
  ...
  this.addAction('toFront', function (editor) {
    var graph = editor.graph
    graph.orderCells(false)
  })
  this.addAction('toBack', function (editor) {
    var graph = editor.graph
    console.log(graph.orderCells())
