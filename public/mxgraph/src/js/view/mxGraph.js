...
mxGraph.prototype.sizeDidChange = function () {
  ...
  if (this.container != null) {
    ...
    var width = Math.max(
      0,
      bounds.x + bounds.width + 2 * border * this.view.scale
    )
    var height = Math.max(
      0,
      bounds.y + bounds.height + 2 * border * this.view.scale
    )
...
mxGraph.prototype.resizeCells = function (cells, bounds, recurse) {
  ...
  try {
    ...
    this.fireEvent(
      new mxEventObject(mxEvent.RESIZE_CELLS, 'cells', cells, 'bounds', bounds)
    )
  } finally {
    this.model.endUpdate()
  }
 ...
 */
mxGraph.prototype.zoomIn = function (factor) {
  this.zoom(1, true)
 ...
 */
mxGraph.prototype.zoomOut = function (factor) {
  this.zoom(1, true)
  this.zoom(factor, true)
}
...
mxGraph.prototype.center = function (horizontal, vertical, cx, cy) {
  ...
  console.log(t.x)
  console.log(bounds.x * s)
  if (!hasScrollbars) {
    console.log(Math.floor(t.x - bounds.x * s + (dx * cx) / s))
...
mxGraph.prototype.zoom = function (factor, center) {
  ...
  if (this.keepSelectionVisibleOnZoom && state != null) {
    ...
    if (center && !hasScrollbars) {
      ...
      if (factor > 1) {
        var f = (factor - 1) / (oldscale * factor * 2)
        dx *= -f
        dy *= -f
      } else {
        var f = (1 / factor - 1) / (oldscale * 2)
        dx *= f
        dy *= f
      }
